	...
	<form action="<?php echo esc_url( home_url() . $_SERVER['REQUEST_URI'] ); ?>" method="post" id="wmd-listings" class="standard-form base">
		...
		<div>
			...
			<div class="media-upload">
				...
				<input type="text" name="wmd[logo]" id="logo" value="<?php echo esc_attr( $logo[0] ); ?>" />
		...
		<div>
			...
			<div class="media-upload">
				<?php foreach ( $data['portfolio'] as $id => $image ) : ?>
					<input type="text" name="wmd[portfolio][<?php echo esc_attr( $id ); ?>]" value="<?php echo esc_attr( $image ); ?>" />
		...
		<div>
			...
			<input type="url" name="wmd[url]" id="url" value="<?php echo esc_url( $data['url'] ); ?>" />
		</div>
		<div>
			<label>Price Range</label>
			<div class="two-column-form">
				<input type="number" name="wmd[low_price]" class="price-low" value="<?php echo esc_attr( $data['low_price'][0]->slug ); ?>" /> to
				<input type="number" name="wmd[high_price]" class="price-high" value="<?php echo esc_attr( $data['high_price'][0]->slug ); ?>" />
			</div>
		</div>
		<div>
			...
			<div class="two-column-form">
				<?php if ( empty( $data['locations'] ) ) : ?>
					<input type="text" name="wmd[state]" class="state" placeholder="State" />,
					<input type="text" name="wmd[city]" class="city" placeholder="City" />
				<?php else : ?>
		...
		<div>
			...
			<input type="text" name="wmd[industry][new]" placeholder="Add New Industry" class="add-new" />
		</div>
		<div>
			...
			<input type="text" name="wmd[tech][new]" placeholder="Add New Technology" class="add-new" />
		</div>
		<div>
			...
			<input type="text" name="wmd[type][new]" placeholder="Add New Service" class="add-new" />
		</div>
		<div>
