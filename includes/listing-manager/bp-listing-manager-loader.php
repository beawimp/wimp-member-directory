...
/**
 * Class BP_Listing_Manager_Component
 *
 * Defines the Member Listing Manager BuddyPress Component by extending the BP_Component object
 *
 ...
 */
class BP_Listing_Manager_Component extends BP_Component {
	...
	function __construct() {
		...
		// Places the component into the active components array so functions like `bp_is_active( 'listing_manager' )`
	...
	function setup_globals( $args = array() ) {
		...
		if ( ! defined( 'BP_WIMP_LISTING_SLUG' ) ) {
			define( 'BP_WIMP_LISTING_SLUG', $this->id );
		}

		// Set up the $globals array to be passed along to parent::setup_globals()
		$globals = array(
			'slug' => BP_WIMP_LISTING_SLUG,
			'root_slug' => isset( $bp->pages->{$this->id}->slug ) ? $bp->pages->{$this->id}->slug : BP_WIMP_LISTING_SLUG,
			'has_directory' => false,
		);
	...
	function setup_nav( $main_nav = array(), $sub_nav = array() ) {
		...
		$main_nav = array(
			'name' => 'Listing',
			'slug' => bp_get_wimp_listing_slug(),
			'position' => 81,
			'screen_function' => 'bp_wimp_listing_screen',
			'default_subnav_slug' => 'listing',
		...
		$sub_nav[] = array(
			'name' => 'Listing',
			'slug' => 'listing',
			'parent_url' => trailingslashit( bp_loggedin_user_domain() . bp_get_wimp_listing_slug() ),
			'parent_slug' => bp_get_wimp_listing_slug(),
			'screen_function' => 'bp_wimp_listing_screen',
			'position' => 10,
		);
 ...
 *
 * By hooking to bp_loaded, we ensure that BP_Example_Component is loaded after Buddypress's core components.
 * This is a good thing because it gives us access to those components' functions and data, should our component
 * interactive with them.
 *
 * @global obj $bp
 */
function bp_wimp_listing_load_core_component() {
	global $bp;

	$bp->wimp_listing = new BP_Listing_Manager_Component;
}
add_action( 'bp_loaded', 'bp_wimp_listing_load_core_component' );